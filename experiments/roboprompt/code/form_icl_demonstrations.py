...
try:
    ...
    # Fallback for different PyRep installations
    try:
        from PyRep.pyrep.objects import VisionSensor
    except ImportError:
        print("❌ PyRep not found. Please install PyRep for RLBench integration.")
        VisionSensor = None
...
ROOT = "/home/aryannzzz/roboprompt/data"   # <- leave as is if data is inside /data
task_name = "pick_place_tape"                  # <- must match the folder name you generated

# UPDATED: Much cleaner and more direct system prompt
...
def form_obs(
    ...
    if gripper_pos is not None:
        gripper_voxel = list(point_to_voxel_index(gripper_pos))
        real_name_to_avg_coord["gripper"] = gripper_voxel
...
class base_task_handler:
    ...
    def save_in_context_demonstrations(self):
        ...
        train_demos = get_stored_demos(ROOT, type(self).__name__, 10, self.sim_name_to_real_name)
...
class push_button(base_task_handler):
    def __init__(self):
        sim_name_to_real_name = {
            "push_button_target": "button_target",
            "target_button_topPlate": "button_top",
            "target_button_wrap": "button_wrap",
        }
        super().__init__(sim_name_to_real_name)

# All other task handlers remain the same...
...
class pick_up_cup(base_task_handler):
    def __init__(self):
        # Map simulator object names to human-readable names
        sim_name_to_real_name = {
            "cup": "cup",           # <-- use the actual RLBench object names if different
            "cup2": "cup_distractor",          # optional if you want distractor info
    	    "cup1_visual": "cup_visual",       # optional visual-only names if useful
    	    "cup2_visual": "cup_distractor_visual",
            "table": "table"        # optional if you want table context
        }
        super().__init__(sim_name_to_real_name)

class push_button_then_pick_up_cup(base_task_handler):
    def __init__(self):
        # Combined object mapping for both tasks
        sim_name_to_real_name = {
            # Push button objects
            "push_button_target": "button_target",
            "target_button_topPlate": "button_top", 
...
class pick_place_tape(base_task_handler):
    def __init__(self):
        # Map your object names - adjust based on your actual objects
        sim_name_to_real_name = {
            "tape": "tape",
    ...
    def get_user_prompt(self, mask_dict, mask_id_to_sim_name, point_cloud_dict):
        # Use the HuggingFace dataset demonstrations instead of RLBench
        mask_id_to_real_name = {mask_id: self.sim_name_to_real_name[name] for mask_id, name in mask_id_to_sim_name.items()
                            if name in self.sim_name_to_real_name}
        obs = form_obs(mask_dict, mask_id_to_real_name, point_cloud_dict)

        # Load from your generated RoboPrompt demonstrations
        hf_demo_path = "./roboprompt_data/0.txt"  # Path to your generated file
        if os.path.exists(hf_demo_path):
            demonstration = open(hf_demo_path, "r").read()
            return demonstration + obs + ">"
        else:
            raise FileNotFoundError(f"Could not find HuggingFace demonstrations at {hf_demo_path}")
...
task_name_to_handler = {"close_jar": close_jar,
                        ...
                        "pick_place_tape": pick_place_tape,
...
if __name__ == "__main__":
    handler = create_task_handler("pick_up_cup")
